...
import { answerTypeSegments } from 'data'
import { FC } from 'react'
import { Question } from 'src/core/interface'
import CreateButton from '../CreateButton'

const QuestionCreate: FC<IProps> = (p) => {
	const { item, changeValueHandler, index, changeTypeHandler, createQuestionAnswer, changeAnswerValueHandler, destroyAnswerHandler } = p

	return (
		<QuestionLayout>
			<div className='flex items-center gap-4 '>
				<span className='text-lg font-medium text-blue-600'>{index}</span>
				<Input className='text-lg font-medium' value={item.title} onChange={(value) => changeValueHandler(value, item.id)} placeholder='Вопрос' />
			</div>
			<div className='mt-4'>
				<Segment onChange={(type) => changeTypeHandler(type, item.id)} value={item.answers.type} items={answerTypeSegments} />
			</div>
			<div className='my-4'>
				<Divider />
			</div>
			{item.answers.type === AnswersType.TEXT ? (
				<div>
					<Input className='font-regular' value={''} onChange={(value) => null} placeholder='Введите правильный ответ' />
				</div>
			) : (
				<div className='pl-12 relative'>
					<div className='flex flex-col gap-1'>
						{item.answers.arr.map((el, index) => (
							<div key={el.id} className='flex items-center gap-3 '>
								<div className='flex items-center  flex-1'>
									{item.answers.type === AnswersType.CHECKBOXS && <Checkbox name={'create_' + item.id.toString()} id={el.id.toString()}></Checkbox>}
									{item.answers.type === AnswersType.RADIO_BUTTONS && <Radio name={'create_' + item.id.toString()} id={el.id.toString()}></Radio>}
									{item.answers.type === AnswersType.BUTTONS && <Radio name={'create_' + item.id.toString()} id={el.id.toString()}></Radio>}
									<div className='pt-1   w-full'>
										<Input
											className='font-regular'
											value={el.title}
											onChange={(value) => changeAnswerValueHandler(value, item.id, el.id)}
											placeholder={`Ответ ${index + 1}`}
										/>
									</div>
								</div>

								<div onClick={() => destroyAnswerHandler(item.id, el.id)}>
					...
					<div className='absolute left-0'>
						<CreateButton className='w-7 h-7' onClick={() => createQuestionAnswer(item.id)} />
					</div>
